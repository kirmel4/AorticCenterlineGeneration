...
from aaa.losses.interface import SegmentationLossTemplate
from aaa.losses.checkers import _average_value_checker, _reduction_value_checker, _3d_shape_asserter
...
class ConfusionLossTemplate(SegmentationLossTemplate):
    ...
    def _get_confusions(self, y_masks_batch, pred_probs_batch):
        tp = torch.sum(y_masks_batch * pred_probs_batch, dim=(1, 2, 3))
        fp = torch.sum(( 1 - y_masks_batch ) * pred_probs_batch, dim=(1, 2, 3))
        fn = torch.sum(y_masks_batch * ( 1 - pred_probs_batch ), dim=(1, 2, 3))
        tn = torch.sum(( 1 - y_masks_batch ) * ( 1 - pred_probs_batch ), dim=(1, 2, 3))
